...
// shorten the list if we are loading - trial version of IBM Cloud 
...
function createServer() {
  ...
  server.get('/api/createEvent', (req, res) => {
    ...
    console.log("createEvent not valid for v2 SDK")
  ...
  server.get('/api/search', (req, res) => {
    const { query, filters, count, returnPassages, sort, queryType } = req.query; // not sure why this isn't working properly
    var params = {};
    ...
    discovery.query(searchParams)
      .then(response => res.json(response))
      .catch(error => {
        console.log("An error occurred in discovery.query: ", error);
        res.status(error.code).json(error);
      });
      // .catch(error => {
      //   if (error.message === 'Number of free queries per month exceeded') {
      //     res.status(429).json(error);
      //   } else {
      //     console.log("An error occured in discovery.query: ", error);
      //     res.status(error.code).json(error);
      //   }
      // });
  });
