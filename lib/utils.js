...
const sortKeys = [
  { type: 'HIGHEST', 
    sortBy: '-result_metadata.confidence', 
    sortByInt: '-confidence',
...
function formatData(rawData, passages, filterString) {
  ...
  for (var index=0; index<data.results.length; index++) {
    ...
    if (!dataItem.enriched_text ||
        !dataItem.enriched_text[0].sentiment) {
    ...
    var newResult = {
      id: dataItem.document_id, // set as doc id to prevent rendering issues
      ...
      date: dataItem.date,
      confidence: dataItem.result_metadata.confidence,
      // score: dataItem.result_metadata.score,
      sentimentScore: dataItem.enriched_text[0].sentiment.score,
      sentimentLabel: dataItem.enriched_text[0].sentiment.label,
      highlight: {
        showHighlight: false,
          ...
          if (typeof entity.mentions !== 'undefined' && entity.mentions.length > 0) {
            entity.mentions.forEach(function(mention) {
              ...
              var highlightText = 'enriched_text.entities.text::"' +
                dataItem.text[0].substr(mention.location[0], mention.location[1] - mention.location[0]) + '"';
