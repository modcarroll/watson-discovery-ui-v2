...
/**
 * Define the Discovery project.
 * @return {Promise} Promise with resolve({project}) or reject(err).
 */
WatsonDiscoverySetup.prototype.findDiscoveryProject = function(params) {
  return new Promise((resolve, reject) => {
    this.discoveryClient.listProjects(params, (err, data) => {
      if (err) {
        console.log("error 2");
        console.error(err);
        return reject(new Error('Failed to get Discovery projects.'));
      } else {
        const projects = data.result.projects;
        // If a DISCOVERY_PROJECt_ID is set, validate it and use it (or fail).
        const validateID = process.env.DISCOVERY_PROJECT_ID;
        // Otherwise, look (by name) for one that we already created.
        const DISCOVERY_PROJECT_NAME = process.env.DISCOVERY_PROJECT_NAME || params.default_name;
        // Otherwise we'll reuse an existing project, if we find a usable one.
        let reuseEnv;

        for (let i = 0, size = projects.length; i < size; i++) {
          const project = projects[i];
          if (validateID) {
            if (validateID === project.project_id) {
              console.log('Found Discovery projects using DISCOVERY_PROJECT_ID.');
              console.log(project);
              params.projectId = project.project_id;
              return resolve(params);
            }
          } else {
            if (project.name === DISCOVERY_PROJECT_NAME) {
              console.log('Found Discovery project by name.');
              console.log(project);
              params.projectId = project.project_id;
              return resolve(params);
            } else if (!project.read_only) {
              reuseEnv = project;
            }
          }
        }
        if (validateID) {
          return reject(new Error('Configured DISCOVERY_PROJECT_ID=' + validateID + ' not found.'));
        } else if (reuseEnv) {
          console.log('Found existing Discovery project to use: ', reuseEnv);
          params.projectId = reuseEnv.projectId;
          return resolve(params);
        }
        // Not found by ID or name or reuse stategy.
        // Set the expected name, so when one is created we will find it.
        params.project_name = DISCOVERY_PROJECT_NAME;
        return resolve(params);
      }
    });
  });
...
/** Create a Discovery project if we did not find one.
 * If an project is passed in, then we already have one.
 * When we create one, we have to create it with our known name
 * so that we can find it later.
 * @param {Object} params - Object describing the project we found or need.
 * @return {Promise} Promise with resolve(project) or reject(err).
 */
WatsonDiscoverySetup.prototype.createDiscoveryProject = function(params) {
  if (params.projectId) {
    return Promise.resolve(params); // If we have an ID, then the env must exist.
  }
  return new Promise((resolve, reject) => {
    // No existing project found, so create it.
    // NOTE: The number of projects that can be created
    // under a trial Bluemix account is limited to one per
    // organization. That is why have the "reuse" strategy above.
    console.log('Creating discovery project...');
    const createParams = {
      name: params.project_name,
      description: 'Discovery project created by ' + params.default_name,
      size: 'LT' // Use string to avoid default of 1.
    };
    this.discoveryClient.createProject(createParams, (err, data) => {
      if (err) {
        console.log("error 4");
        console.error('Failed to create Discovery project.');
        return reject(err);
      } else {
        console.log('Created Discovery project: ', data);
        params.projectId = data.result.project_id;
        resolve(params);
      }
    });
  });
 ...
 * @return {Promise}
 */
WatsonDiscoverySetup.prototype.loadDiscoveryProject = function(params) {
  return new Promise((resolve, reject) => {
    console.log('Get project to check its status.');
    this.discoveryClient.getProject(params, (err, data) => {
      if (err) {
        console.error(err);
        return reject(err);
      } else {
        ...
        if ((! params.checkedForExistingDocs) && (numDocsLoaded > 0)) {
          ...
          for (let i = 0; i < docCount; i++) {
            ...
            this.discoveryClient.addDocument({projectId: params.projectId,
...
WatsonDiscoverySetup.prototype.setupDiscovery = function(setupParams, callback) {
  console.log("setup Discovery");
  this.findDiscoveryProject(setupParams)
    .then(params => this.createDiscoveryProject(params))
...
WatsonDiscoverySetup.prototype.loadDiscoveryData = function (collectionParams, callback) {
  this.loadDiscoveryProject(collectionParams)
    .then(params => callback(null, params))
    .catch(callback);
};
